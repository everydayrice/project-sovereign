...
export function continuityWorkbench(snapshot) {
  ...
  const projectSelect=(refs=[])=>`<label>Project<select name="project_id"><option value="">Unassigned</option>${(snapshot.projects??[]).map(p=>`<option value="${esc(p.canonical_record_id)}" ${refs.includes(p.canonical_record_id)?'selected':''}>${esc(p.payload?.title??p.scope?.project??'Project')}</option>`).join('')}</select></label>`;

  return `<section class="panel"><h2>Create a task</h2><form method="post" data-workflow="task-create">${projectSelect([snapshot.selected_project_id])}${field('title','Title','',true)}${field('objective','Objective','',true)}${field('next_action','Next action')}<button>Create task</button></form></section>
  <section class="panel"><h2>Tasks and resume</h2>${tasks.length ? tasks.map(task => `<details><summary>${esc(task.title)} · ${esc(task.state)}</summary><p>${esc(task.objective)}</p><form method="post" data-workflow="task-update" data-id="${esc(task.task_capsule_id)}">${projectSelect(task.intelligence_references??[])}${field('title','Title',task.title,true)}${field('objective','Objective',task.objective,true)}${field('next_action','Next action',task.next_action)}<label>Status<select name="state">${states.map(state => `<option ${task.state === state ? 'selected' : ''}>${state}</option>`).join('')}</select></label><label>Blockers, one per line<textarea name="blockers">${esc(task.blockers?.join('\n'))}</textarea></label><button>Save task</button></form><button type="button" data-resume="${esc(task.task_capsule_id)}">View resume</button> <button type="button" data-start="${esc(task.task_capsule_id)}">Start work</button><div data-resume-output></div></details>`).join('') : '<p>No tasks yet.</p>'}</section>
...
function technicalDetails(value, label = 'Technical details') {
 return `<details class="technical"><summary>${esc(label)}</summary><pre>${esc(JSON.stringify(value,null,2))}</pre></details>`;
}
function knowledgeTitle(payload, fallback) { return payload?.title ?? payload?.subject ?? payload?.name ?? fallback; }
function knowledgeBody(payload) {
 if (!payload || typeof payload !== 'object') return `<p>${esc(payload)}</p>`;
 return Object.entries(payload).filter(([key])=>!['title','subject','name'].includes(key)).map(([key,value])=>`<div class="knowledge-field"><span>${esc(key.replaceAll('_',' '))}</span><p>${esc(typeof value === 'object' ? readable(value) : value)}</p></div>`).join('');
}
function evidenceLinks(provenance = []) {
 return provenance.length ? `<details class="evidence"><summary>Sources · ${provenance.length}</summary><ul>${provenance.map(ref=>{const value=typeof ref==='string'?ref:null;return `<li>${value && /^https?:\/\//i.test(value)?`<a href="${esc(value)}" target="_blank" rel="noopener noreferrer">${esc(value.split('/').pop() || 'View source')}</a>`:esc(readable(ref))}</li>`;}).join('')}</ul></details>` : '';
}
export function intelligenceWorkbench(snapshot) {
 const changes=snapshot.canonical_changes;
 const pending=changes.filter(({change_set:c})=>['pending_approval','ready'].includes(c.state));
 const past=changes.filter(({change_set:c})=>!['pending_approval','ready'].includes(c.state));
 const review=({change_set:c,operations})=>`<article class="review-card"><div class="panel-header"><h3>${esc(c.title)}</h3><span class="badge">${esc(c.state.replaceAll('_',' '))}</span></div><p class="muted">${esc(c.reason)}</p><p>${operations.length} proposed change${operations.length===1?'':'s'}</p>${operations.map(op=>{const record=op.replacement??op.patch??{};return `<section class="change-preview"><span class="badge">${esc(op.operation_type)}</span><h4>${esc(knowledgeTitle(record.payload,'Record change'))}</h4>${knowledgeBody(record.payload??record)}${evidenceLinks(record.provenance)}</section>`;}).join('')}${evidenceLinks(c.provenance)}${technicalDetails({change_set:c,operations},'Audit and technical details')}<div class="review-actions">${['pending_approval','ready'].includes(c.state)?`<button class="primary" data-canon-action="${c.provenance?.some(p=>p.candidate_intelligence_id)?'approve-candidate':'approve'}" data-id="${esc(c.canonical_change_set_id)}">Approve and apply</button><button data-canon-action="reject" data-id="${esc(c.canonical_change_set_id)}">Reject change</button>`:c.state==='applied'?`<button data-canon-action="revert" data-id="${esc(c.canonical_change_set_id)}">Propose reversing revision</button>`:''}</div></article>`;
 return `<nav class="section-nav" aria-label="Knowledge views" data-knowledge-nav><a href="#knowledge-records">Saved knowledge</a><a href="#knowledge-review">Review (${pending.length})</a><a href="#knowledge-search">Search</a><a href="#knowledge-health">Health</a><a href="#knowledge-candidates">Candidates (${snapshot.candidates.length})</a><a href="#knowledge-history">History</a></nav>
 <section class="panel" id="knowledge-review"><div class="panel-header"><div><h2>Ready for your review</h2><p class="muted">Approve proposed changes to make them part of your trusted knowledge.</p></div><span class="badge">${pending.length} pending</span></div>${pending.map(review).join('')||'<p class="empty">You’re all caught up. No changes need approval.</p>'}</section>
 <section class="panel" id="knowledge-records"><h2>Saved knowledge</h2><p class="muted">Approved records with their sources and preserved history.</p>${snapshot.canonical_records.map(({record,revisions})=>`<details class="knowledge-record"><summary>${esc(knowledgeTitle(record.payload,record.record_type+' record'))}<span class="record-meta">${esc(record.record_type)} · ${esc(record.lifecycle_state)}</span></summary>${knowledgeBody(record.payload)}${evidenceLinks(record.provenance)}${technicalDetails({record,revisions},'Record details and revision history')}</details>`).join('')||'<p class="empty">Approved knowledge will appear here.</p>'}</section>
 <section class="panel" id="knowledge-search"><h2>Search your evidence</h2><form method="post" data-workflow="search"><label>Search terms<input name="q" placeholder="Find a decision, project or source…" required></label><label>Source<select name="source_id"><option value="">All sources and approved knowledge</option>${snapshot.sources.sources.map(source=>`<option value="${esc(source.source_id)}">${esc(source.display_name)}</option>`).join('')}</select></label><button>Search</button></form><div id="search-results"></div></section>
 <section class="panel" id="knowledge-health"><h2>Knowledge health</h2><p>${snapshot.canon_check.possible_conflicts.length} possible conflicts · ${snapshot.canon_check.source_issues.length} source issues · ${snapshot.canon_check.uncertainties.length} uncertain records</p><p class="muted">${esc(snapshot.canon_check.coverage)}</p>${technicalDetails(snapshot.canon_check,'Inspect health details')}</section>
 <section class="panel" id="knowledge-candidates"><details open><summary>Candidate inbox <span class="badge">${snapshot.candidates.length}</span></summary><p class="muted">Unapproved suggestions from your sources. Review a candidate before proposing a change.</p>${snapshot.candidates.map((c,index)=>`<details><summary>${esc(knowledgeTitle(c.payload,'Suggestion '+(index+1)))}<span class="record-meta">${esc(c.record_type)} · ${esc(c.state)}</span></summary>${knowledgeBody(c.payload)}${evidenceLinks(c.provenance)}${technicalDetails(c)}${c.state==='proposed'?`<div class="review-actions"><button data-candidate-action="propose-canonical" data-id="${esc(c.candidate_intelligence_id)}">Propose canonical change</button><button data-candidate-action="reject" data-id="${esc(c.candidate_intelligence_id)}">Reject candidate</button></div>`:''}</details>`).join('')||'<p>No suggestions yet.</p>'}</details></section>
 <section class="panel" id="knowledge-history"><details open><summary>Change history <span class="badge">${past.length}</span></summary>${past.map(item=>`<details><summary>${esc(item.change_set.title)}<span class="record-meta">${esc(item.change_set.state)} · revision ${esc(item.change_set.resulting_canonical_revision??'—')}</span></summary>${review(item)}</details>`).join('')||'<p>No changes yet.</p>'}</details></section>`;
}
...
const WORKFLOW_CLIENT = String.raw`function workflowClient(focus) {
  if(focus==='intelligence' && document.querySelectorAll) {
    const links=[...document.querySelectorAll('[data-knowledge-nav] a')];
    const panels=links.map(link=>document.getElementById(link.hash.slice(1))).filter(Boolean);
    const selectView=()=>{
      const target=links.find(link=>link.hash===location.hash)?.hash ?? '#knowledge-records';
      panels.forEach(panel=>{panel.hidden=('#'+panel.id)!==target;});
      links.forEach(link=>{if(link.hash===target)link.setAttribute('aria-current','page');else link.removeAttribute('aria-current');});
    };
    if(links.length){selectView();window.addEventListener('hashchange',selectView);}
  }
  let sessionId=null;
