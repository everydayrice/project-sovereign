  ...
  async fetch(request, env = {}) {
    try {
      ...
      if (request.method === "POST" && url.pathname === "/v1/ask") {
        ...
        const result = await retrieval.ask({
          tenantId: binding.tenant_id,
          query: body.query,
      ...
      if (request.method === "POST" && initializeMatch) {
        const result = await processStoredSource({
          request,
      ...
      if (request.method === "POST" && candidateProposeMatch) {
        return await handleCandidateProposal({ request, candidateId: decodeURIComponent(candidateProposeMatch[1]), authenticate, persistence });
      }
      ...
      if (request.method === "POST" && candidateRejectMatch) {
        return await handleCandidateRejection({ request, candidateId: decodeURIComponent(candidateRejectMatch[1]), authenticate, persistence });
      }
      ...
      if (request.method === "POST" && candidateApprovalMatch) {
        return await handleCandidateApproval({ request, changeSetId: decodeURIComponent(candidateApprovalMatch[1]), authenticate, persistence });
      }

      const response = await baseWorker.fetch(request, env);
      if (request.method === "GET" && url.pathname === "/console/intelligence" && response.ok) {
        return injectAskSovereign(response);
      }
      return response;
    } catch (error) {
...
async function handleUploadWithAutomaticProcessing({ request, env, authenticate, persistence, files }) {
  ...
  if (sourceId) {
    processing = await processStoredSource({ request, sourceId, authenticate, persistence, files, tolerateUnsupported: true });
  }
...
async function processStoredSource({ request, sourceId, authenticate, persistence, files, tolerateUnsupported = false }) {
  ...
  if (sourceItems.length !== 1) {
    throw new SovereignError("source_item_required", "Automatic direct-upload processing currently expects one stored file per source.", { status: 409 });
  }
  const item = sourceItems[0];
  if (item.storage_state !== "stored") {
    throw new SovereignError("source_object_not_stored", "Source content must be stored in R2 before processing.", { status: 409 });
  }
  ...
  if (!supportsAutomaticTextIngestion({ fileName: item.display_name, mimeType: item.mime_type })) {
    if (tolerateUnsupported) {
      return {
        ...
        analyzed: false,
        canonicalized: false,
        processing_note: "stored_unparsed_format"
      };
    }
  ...
  for (const candidate of ingestion.candidates) {
    const duplicate = platform.store.list("candidateIntelligence", (existing) =>
      existing.tenant_id === tenantId &&
      existing.payload?.statement === candidate.payload?.statement &&
      existing.provenance?.some((entry) => entry.source_item_id === item.source_item_id)
    )[0];
  ...
  platform.store.update("sourceItems", item.source_item_id, (current) => ({
    ...
    metadata: {
      ...current.metadata,
      parser_key: ingestion.parser,
      parser_version: ingestion.parser_version,
      chunk_count: ingestion.chunks.length,
      normalized_text_length: ingestion.normalized_text_length,
...
async function injectAskSovereign(response) {
  ...
  const updated = text.replace("</header>", `</header>${panel}`);
