...
test('Tenant export excludes other tenants, uninstalled catalog entries and authentication identities',()=>{
 ...
 const owner=p.command.createPrincipal({tenantId:a.tenant_id,displayName:'Owner',authSubjectReference:'do-not-export-auth-binding'});
 const actor=p.command.resolveActor({tenantId:a.tenant_id,principalId:owner.principal_id,actor:{provider:{key:'test'},surface:{key:'test',type:'cli'},externalSessionId:'export-context'}});
 ...
 assert.equal(result.data.actorInstances[0].actor_instance_id,actor.actor_instance_id);assert.equal(result.data.tenants.length,1);assert.equal(result.data.principals.length,1);assert.equal(result.data.extensions.length,0);assert.equal(result.data.ideas.length,0);assert.ok(!JSON.stringify(result).includes('do-not-export-auth-binding'));
});
